...
function check(log, validity, req, res, type, full, finalValidator, callback) {
  ...
  if (!req.body) {
    callback(new ValidationFailureError('Empty Request Body'));
    return;
  }
 ...
 * @param {Function} errHandler A function which is called with (err) if the
 * requets body validation fails.
...
exports.attach = function attachValidatorMiddleware(validity, errHandler, options) {
  return function addValidator(req, res, next) {
    ...
    req.checkAndOnSuccess = checkAndOnSuccess(log, validity, req, res, true, errHandler);
    req.checkAndOnPartialSuccess = checkAndOnSuccess(log, validity, req, res, false, errHandler);
